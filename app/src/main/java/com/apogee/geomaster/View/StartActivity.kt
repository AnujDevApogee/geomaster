...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    registerReceiver(receiver,  IntentFilter("GET_SERVER_DATA"))  //<----Register
...
override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
    ...
    if (requestCode == RequestPermissionCode) {
        if (grantResults.size > 0) {
            ...
            val RExternalStoragePermission = grantResults[1] == PackageManager.PERMISSION_GRANTED
            val WExternalStoragePermission = grantResults[2] == PackageManager.PERMISSION_GRANTED
            ...
            if (LocationPermission && RExternalStoragePermission && WExternalStoragePermission && Location2Permission && ntwrkstatePermission) {
                Toast.makeText(this@StartActivity, "Permission Granted", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(this@StartActivity, "Permission Denied", Toast.LENGTH_LONG).show()
            }
        }
    }
}
...
fun CheckingPermissionIsEnabledOrNot(): Boolean {
    val SecondPermissionResult = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.ACCESS_FINE_LOCATION)
    val ThirdPermissionResult = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.READ_EXTERNAL_STORAGE)
    val ForthPermissionResult = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.WRITE_EXTERNAL_STORAGE)
    val FifthPermissionResult = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.ACCESS_COARSE_LOCATION)
    val sixthPermissionResult = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.ACCESS_NETWORK_STATE)
